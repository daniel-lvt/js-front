...
export default function BasicGrid({ ...args }) {
    const { departamentData = [], amountData = [], dataDB = [] } = args;
    ...
    const onClickEvent = () => {
        const dataResult = lodash.groupBy((dataFilter.length ? dataFilter : dataDB), 'CANTIDAD')
        const dataObject = (amountData || []).map((key, idx) => ({
            prop: {
                label: key,
    ...
    return (
        <Box sx={{ flexGrow: 1 }}>
            <Div>Accidentes producidos por Armas Medio</Div>
